export type Tool = 'arrow' | 'rect' | 'circle' | 'pen' | 'highlight' | 'text' | 'eraser';
...
export interface TextBoxShape   { type: 'textbox';   x: number;  y: number;  w: number;   h: number;  text: string; color: string; bgColor: string; bgOpacity: number; fontSize: number; fontFamily: string; bold: boolean; italic: boolean; underline: boolean; }
export type Shape = ArrowShape | RectShape | CircleShape | PenShape | HighlightShape | TextShape | TextBoxShape;
